...
## Controls
- **Ctrl + Middle click**: capture origin (mortar position). **Shift + Middle click**: capture target. **F9**: clear. F8/F7 are keyboard aliases for origin/target. A plain middle click is the game's own ping and never triggers SITREP (the button state is only polled to detect the Ctrl/Shift chord edge; nothing is consumed or synthesized).
