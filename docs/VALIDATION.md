...
- CI fix (this branch): verify was red on main since introduction — NU1004 because src/Sitrep.Core/packages.lock.json carries a net10.0/win-x64 section (from publish) while plain locked restore evaluates Core with no RID. Fixed by declaring <RuntimeIdentifiers>win-x64</RuntimeIdentifiers> in Sitrep.Core.csproj so every restore context matches the committed lock file. Verified locally: `dotnet restore Sitrep.slnx --locked-mode` (with EnableWindowsTargeting for the Linux box), Core locked restore with and without -r win-x64, full Release build, 72/72 tests, and locked-mode win-x64 self-contained publish all pass; lock file unchanged.
